...
```
![image](https://user-images.githubusercontent.com/58906858/211701364-6605fa2b-0886-4963-9e67-aa3e86d0cd44.png)
...
#### todo 앱 프로젝트를 하면서 배우고 느낀점
```
앞에서 만든 앱 프로젝트는 클라이언트 수준의 앱이였다면 todo 앱 프로젝트는 Google사의 Firestore 데이터베이스를 활용한
서버와 클라이언트를 모두 활용하는 CRUD 기능의 앱이라는 점에서 차이가 있었습니다.
streamBuilder를 사용하여 서버의 데이터를 수정,추가,삭제하면서 앱에 바로바로 갱신할 수 있었습니다.
클라이언트 앱과 서버,클라이언트 앱의 가장 큰 차이는 데이터의 휘발성과 관련이 있습니다.
서버,클라이언트 앱은 데이터를 서버에 저장하여 앱을 종료하여도 데이터가 없어지지 않지만
클라이언트는 앱을 종료하면 데이터를 모두 잃습니다. 
현재 앱 시장에 출시되는 대부분의 앱은 서버와 클라이언트를 모두 운용할 줄 알아야 합니다.
단순한 CRUD 기능의 앱이지만 향후에 만들 앱 프로젝트들에 데이터베이스를 활용할 수 있게 되어서 도움이 되었습니다.
사용하기 간편한 수준의 실시간 Firestore 데이터베이스뿐만 아니라 계층형, 관계형, 네트워크형 데이터베이스 등 
다양한 데이터베이스를 학습하여 적용해봐야겠다고 생각했습니다.
```
